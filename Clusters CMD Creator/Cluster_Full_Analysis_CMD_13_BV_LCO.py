...
# We define both, and prompt only the relevant one (per cluster type).
G_TUNING = 5
O_TUNING = 5
if cluster_type == "G":
...
# ---- Parameter selection by cluster type and distance (+ strictness) ----
def _set_params_by_type_and_distance(cluster_type: str, dist_pc: float):
    """
    Sets global thresholds for membership using one master 'strictness' knob:
      - 1 = very tight (few members, central, high-confidence)
      - 10 = very loose (many members, includes outskirts/field)
    Notes:
      * All parameters below include concise comments explaining their effect.
      * Alignment parameters are kept fixed for robustness (do NOT tie them to the knob).
    """
    g = globals()
    global G_TUNING, O_TUNING

    # map user knob to safe scale s: 1→0.2, 5→1.0 (baseline), 10→2.0
    ...
    if cluster_type.upper() == 'G':
        # ---- Globular cluster (baseline = your v6 values; scaled by s) ----
        g['MATCH_MAX_ARCSEC']    = 2.0  * s   # Max catalog–image match radius (arcsec). Smaller → fewer false matches; larger → more matches but risk mismatches.
        g['PARALLAX_MIN_HALF']   = 0.05 * s   # Inner parallax half-width (mas) around π0. Smaller → tighter selection near the cluster parallax.
        g['PARALLAX_MAX_HALF']   = 5.00 * s   # Outer parallax half-width (mas). Lower → excludes distant/near interlopers; higher → admits more.
        g['PARALLAX_FRAC']       = 100.0 * s  # Relative expansion (% of π0). 100 keeps cap effectively off; scaling follows knob.
        g['PARALLAX_ERR_SIGMA']  = 5.0  * s   # Allowed parallax error (σ, Gaia). Lower → accept only high-S/N parallaxes.
        g['PM_SIGMA_FLOOR']      = 1.5  * s   # PM floor dispersion (mas/yr). Lower → tighter PM clump; higher → more permissive.
        g['PM_ERR_SIGMA']        = 8.0  * s   # Allowed PM error (σ). Lower → stricter; higher → looser (may admit field stars).
        g['PM_CHI2']             = 40.0 * s   # PM coherence threshold (χ²). Lower → stricter clustering; higher → looser.
        g['SPATIAL_CORE_FRAC']   = max(0.05, min(0.40, 0.15 * s))  # Core radius fraction of the field for reference stats. Lower → emphasizes central stars.
        g['ALIGN_SEARCH_ARCSEC'] = 60.0       # Alignment search radius (arcsec). Kept fixed for stability of WCS alignment.
        g['ALIGN_MIN_PAIRS']     = 20         # Min matched pairs for alignment. Kept fixed.

    else:
        # ---- Open cluster (looser baseline; scaled by s) ----
        g['MATCH_MAX_ARCSEC']    = 8.0  * s   # Max catalog–image match radius (arcsec). Bigger → looser matching; smaller → more precise, fewer pairs.
        g['PARALLAX_MIN_HALF']   = 0.10 * s   # Inner parallax half-width (mas). Smaller → tighter window near π0; higher risk of missing members if too small.
        g['PARALLAX_MAX_HALF']   = 1.00 * s   # Outer parallax half-width (mas). Lower cuts field interlopers; higher admits more.
        g['PARALLAX_FRAC']       = max(5.0, 20.0 * s)  # Relative expansion (% of π0). Lower = window sticks closer to π0.
        g['PARALLAX_ERR_SIGMA']  = 4.0  * s   # Allowed parallax error (σ). Lower = stricter (cleaner); higher = looser (more stars).
        g['PM_SIGMA_FLOOR']      = 1.0  * s   # PM floor dispersion (mas/yr). Lower = tighter PM cluster; higher = more permissive.
        g['PM_ERR_SIGMA']        = 4.0  * s   # Allowed PM error (σ). Lower = stricter PM quality; higher = looser.
        g['PM_CHI2']             = 25.0 * s   # PM coherence threshold (χ²). Lower = stricter clustering; higher = looser (more members).
        g['SPATIAL_CORE_FRAC']   = max(0.05, min(0.40, 0.15 * s))  # Core radius fraction used for stats. Lower = more central weighting.
        g['ALIGN_SEARCH_ARCSEC'] = 40.0       # Alignment search radius (arcsec). Kept fixed.
        g['ALIGN_MIN_PAIRS']     = 25         # Min matched pairs for alignment. Kept fixed.
...
if __name__ == "__main__":
    ...
    # Set a calibrated magnitude for downstream plots
    if 'Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any():
    ...
    if 'Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any():
        ...
        mask_color = np.isfinite(df_det['B_minus_V'])
    ...
    try:
        ...
        color_col = 'B_minus_V' if ('Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any()) else 'B_minus_G'
mag_col = 'Mag_V' if ('Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any()) else 'Mag_G'
ax.scatter(df_det.loc[mask_app, color_col], df_det.loc[mask_app, mag_col],
        ...
        ax.set_xlabel('B - V (mag)' if color_col=='B_minus_V' else 'B - G (mag)'); ax.set_ylabel('V (mag)' if mag_col=='Mag_V' else 'G (mag)'); ax.invert_yaxis()
        ax.set_title('CMD: V vs (B - V)' if color_col=='B_minus_V' else 'CMD: G vs (B - G)'); ax.legend(loc='best', frameon=True)
    ...
    if np.isfinite(DM):
        ...
        try:
            fig, ax = plt.subplots(figsize=(7.5, 7.5))
            color_col = 'B_minus_V' if ('Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any()) else 'B_minus_G'
mag_abs_col = 'M_V' if ('Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any()) else 'M_G'
ax.scatter(df_det.loc[mask_abs, color_col], df_det.loc[mask_abs, mag_abs_col],
            ...
            ax.scatter(df_det.loc[mask_abs_mem, color_col], df_det.loc[mask_abs_mem, mag_abs_col],
                       s=14, alpha=0.9, linewidths=0, label='Cluster members')
            ax.set_xlabel('B - V (mag)' if color_col=='B_minus_V' else 'B - G (mag)'); ax.set_ylabel('M_V (mag)' if ('Mag_V' in df_det.columns and np.isfinite(df_det['Mag_V']).any()) else 'M_G (mag)'); ax.invert_yaxis()
            ax.set_title(f'CMD: ' + ('M_V vs (B - V)' if ("M_V" in df_det.columns) else 'M_G vs (B - G)') + f'  [d={CLUSTER_DISTANCE_PC:.0f} pc, DM={DM:.2f} mag, A_V={AG_EXTINCTION:.2f}, E(B-V)={EBG_REDDENING:.2f}]')
