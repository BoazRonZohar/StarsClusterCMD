...
try:
    ...
    E_BV = float(input("Enter Galactic color excess E(B-V): (EXP: 0.0.041)"))
    ...
    if os.path.exists(cal_csv):
        ...
        if os.path.exists(apass_csv):
            ...
            # remove unused APASS columns that end up empty
        drop_cols = ["RA", "DEC", "Mag_B", "Mag_V"]
...
def get_cluster_members(cluster_name, fits_file, match_radius_arcmin=1.0):
    ...
    if "proba" in cluster_members.colnames:
        ...
        raise KeyError("No probability column found in cluster membership table")

    # build DataFrame with unified "Prob" column
    members_df = pd.DataFrame({
        "RA": cluster_members["RA_ICRS"],
        "DEC": cluster_members["DE_ICRS"],
        "Prob": cluster_members[prob_col],
        "X": xpix,
        "Y": ypix
    })
...
try:
    members_df = get_cluster_members(cluster_name, fits_file_B, match_radius_arcmin=1.0)
