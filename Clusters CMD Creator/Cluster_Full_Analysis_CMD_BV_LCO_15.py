# -*- coding: utf-8 -*-
"""
Created on Sun Sep  7 18:24:55 2025
...
# =============================================================================
# (BLOCK 1)  Gaia->(B,V) transforms (no "Giants" compression; use Dwarfs/All)
# =============================================================================
class _Poly:
    """Simple Horner polynomial evaluator."""
    def __init__(self, coefs): self.coefs = tuple(coefs)
    def __call__(self, x):
...
# X = BP-RP
# Y = (V - G), Validity = All   (Pancino+2022 DR3 Table 9; VizieR)
_PV_ALL = _Poly((
    0.02458722322568100, 0.05385533955796798, 0.2187428599449099,
   -0.1871099675498201, 0.01678496838235600, 0.04153625834745960,
   -0.05603722850574500, 0.01633249830098700, -0.00154217904703490, 0.0
))

# Y = (B - V), Validity = Dwarfs  (recommended for open clusters)
_PBV_DWARFS = _Poly((
    0.13917935097371995,
    1.2955706632713593,
    0.797640276621739804,
    -0.4414351059720599,
    0.14528249298055201,
    -0.023654995169922
))

# Y = (B - V), Validity = All  (stable alternative)
_PBV_ALL = _Poly((
    0.02458722322568100,
    0.05385533955796798,
    0.2187428599449099,
...
def gaia_to_BV_inline(df_gaia: pd.DataFrame, regime: str = "auto"):
    """
    Compute V_std, B_std, B_minus_V, BP_RP from Gaia DR3 mags.

    regime: "dwarfs" | "all" | "auto"
        auto = use Dwarfs for BP-RP <= 1.2, otherwise All.
    """
    bp = pd.to_numeric(df_gaia['phot_bp_mean_mag'], errors='coerce')
    rp = pd.to_numeric(df_gaia['phot_rp_mean_mag'], errors='coerce')
    G  = pd.to_numeric(df_gaia['phot_g_mean_mag'],  errors='coerce').to_numpy()
    bp_rp = (bp - rp).to_numpy()

    # V from G + polynomial
    V_minus_G = _PV_ALL(bp_rp)
    V = G + V_minus_G

    # B-V from Dwarfs/All (not Giants)
    if regime == "dwarfs":
        B_minus_V = _PBV_DWARFS(bp_rp)
    elif regime == "all":
        B_minus_V = _PBV_ALL(bp_rp)
    else:  # auto
        mask_dw = bp_rp <= 1.2
        B_minus_V = np.empty_like(bp_rp, dtype=float)
        B_minus_V[mask_dw]  = _PBV_DWARFS(bp_rp[mask_dw])
        B_minus_V[~mask_dw] = _PBV_ALL(bp_rp[~mask_dw])

    B = V + B_minus_V
    return {"V_std": V, "B_std": B, "B_minus_V": B_minus_V, "BP_RP": bp_rp}
# =============================================================================
...
cluster_distance    = _ask("Cluster distance (pc)", 6800.0, float)
...
fits_file_G = _ask("Path to G- or V-band FITS image", _default_GV, str)  # still stored as fits_file_G
...
# (BLOCK 4) input sanity checks for robustness across clusters
if (EBG_REDDENING < 0) or (EBG_REDDENING > 1.0):
    raise ValueError(f"E(B-V) out of range: {EBG_REDDENING}")
if (AG_EXTINCTION < 0) or (AG_EXTINCTION > 10.0):
    raise ValueError(f"A_V out of range: {AG_EXTINCTION}")
...
# =============================================================================
# (BLOCK 2)  Robust zero-point calibration with proper Gaia indexing + diagnostics
# =============================================================================
def calibrate_zero_point_generic(df_det: pd.DataFrame, gaia_df: pd.DataFrame,
                                 m_inst: pd.Series, target_mag,
                                 flux_series: pd.Series, mag_range=None, name=""):
    """
    Solve: target_mag = m_inst + ZP + beta * color  (on matched, non-members).
    Returns: ZP, beta, used_mask(len=df_det), rms, diagnostics(dict)
    """
    import numpy as _np, pandas as _pd

    if ('matched' not in df_det.columns) or ('ClusterMember' not in df_det.columns):
        return _np.nan, _np.nan, _np.zeros(len(df_det), bool), _np.nan, None

    mask = (df_det['matched'] == True) & (df_det['ClusterMember'] == False)
    if mask.sum() < 10: mask = (df_det['matched'] == True)
    if mask.sum() < 5:  return _np.nan, _np.nan, _np.zeros(len(df_det), bool), _np.nan, None
    ...
    BP = pd.to_numeric(gsub.get('phot_bp_mean_mag'), errors='coerce')
    RP = pd.to_numeric(gsub.get('phot_rp_mean_mag'), errors='coerce')
    color = (BP - RP).to_numpy()

    tmag_full = (pd.to_numeric(target_mag, errors='coerce').to_numpy()
                 if isinstance(target_mag, pd.Series)
                 else np.asarray(target_mag, float))
    tmag = tmag_full[idx_gaia] if tmag_full.shape[0] == len(gaia_df) else tmag_full

    F = pd.to_numeric(flux_series.loc[mask], errors='coerce').to_numpy(float)

    q = np.isfinite(tmag) & np.isfinite(color)
    if isinstance(mag_range, (tuple, list)) and len(mag_range) == 2:
        lo, hi = float(mag_range[0]), float(mag_range[1]); q &= (tmag >= lo) & (tmag <= hi)
    if np.isfinite(F).any():
        p1, p99 = np.nanpercentile(F, [5, 99]); q &= (F >= p1) & (F <= p99)
    if q.sum() < 5:
        return np.nan, np.nan, np.zeros(len(df_det), bool), np.nan, None

    y_all = tmag - pd.to_numeric(m_inst.loc[mask], errors='coerce').to_numpy(float)
    y = y_all[q]; x = color[q]
    X = np.vstack([np.ones_like(x), x]).T
    p, *_ = np.linalg.lstsq(X, y, rcond=None)
    resid = y - X @ p
    s = np.nanstd(resid)
    good = (np.abs(resid) <= 3*s) if s > 0 else np.isfinite(resid)
    if good.sum() >= 5:
        p, *_ = np.linalg.lstsq(X[good], y[good], rcond=None)
        resid = y[good] - (X[good] @ p)
        used_local = good
    else:
        used_local = np.ones_like(y, bool)

    ZP, beta = float(p[0]), float(p[1])
    rms = float(np.nanstd(resid)) if resid.size else np.nan

    used_mask = np.zeros(len(df_det), bool)
    used_mask[np.where(mask)[0][q][used_local]] = True

    diag = {
        "name": name, "N_used": int(used_local.sum()),
        "ZP": ZP, "beta": beta, "rms": rms,
        "med_resid": float(np.nanmedian(resid)) if resid.size else np.nan
    }
    return ZP, beta, used_mask, rms, diag
# =============================================================================
...
if __name__ == "__main__":
    ...
    # -- (BLOCK 1 in use) Gaia→(B,V) standards --
    ...
    _BVref = gaia_to_BV_inline(df_gaia_basic, regime="auto")
    Vref = _BVref['V_std']
    Bref = _BVref['B_std']

    # --- Calibrate second band: to V (if user said V) or to Gaia G (fallback) ---
    if gv_filter == "V":
        ZP_V, beta_V, usedV, rmsV, dV = calibrate_zero_point_generic(
            df_det, gaia, df_det['m_inst_G'], target_mag=Vref,
            flux_series=df_det['Flux_G'], mag_range=CALIB_G_MAG_RANGE, name="V"
        )
        print(f"[calib V] {dV}")
        if (not np.isfinite(ZP_V)) or dV is None or dV["N_used"] < 10:
            raise RuntimeError("V zero-point failed; do not fall back to G.")
        ...
        color_all = color_all.fillna(np.nanmedian(color_vals.iloc[df_det.loc[usedV, 'gaia_index'].astype(int)].values)
                                     if usedV.any() else np.nanmedian(color_vals))
        df_det['Mag_V'] = df_det['m_inst_G'] + ZP_V + beta_V * color_all
    else:
        ZP_G, beta_G, usedG, rmsG, dG = calibrate_zero_point_generic(
            df_det, gaia, df_det['m_inst_G'], target_mag=Ggaia,
            flux_series=df_det['Flux_G'], mag_range=CALIB_G_MAG_RANGE, name="G"
        )
        print(f"[calib G] {dG}")
        if np.isfinite(ZP_G):
            color_all = pd.Series(np.nan, index=df_det.index)
            ...
            color_vals = (BPgaia - RPgaia)
            color_all.loc[m] = color_vals.iloc[df_det.loc[m, 'gaia_index'].astype(int)].values
            color_all = color_all.fillna(np.nanmedian(color_vals.iloc[df_det.loc[usedG, 'gaia_index'].astype(int)].values)
                                         if usedG.any() else np.nanmedian(color_vals))
    ...
    ZP_B, beta_B, usedB, rmsB, dB = calibrate_zero_point_generic(
        df_det, gaia, df_det['m_inst_B'], target_mag=Bref,
        flux_series=df_det['Flux_B'], mag_range=CALIB_BP_MAG_RANGE, name="B"
    )
    print(f"[calib B] {dB]")
    if (not np.isfinite(ZP_B)) or dB is None or dB["N_used"] < 10:
        raise RuntimeError("B zero-point failed.")
    color_all = pd.Series(np.nan, index=df_det.index)
    m = (df_det['matched'] == True)
    color_vals = (BPgaia - RPgaia)
    color_all.loc[m] = color_vals.iloc[df_det.loc[m, 'gaia_index'].astype(int)].values
    color_all = color_all.fillna(np.nanmedian(color_vals.iloc[df_det.loc[usedB, 'gaia_index'].astype(int)].values)
                                 if usedB.any() else np.nanmedian(color_vals))
    ...
    # ================= CMDs =================
    # (BLOCK 3) Apparent CMD in V vs (B-V) when V exists; otherwise G vs (B-G)
    if hasV:
    ...
    # Absolute CMD with extinction applied on magnitude axis
