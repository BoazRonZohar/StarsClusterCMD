...
cluster_type     = _ask("Cluster type [O=open, G=globular, GAL=Galaxy]", "O", str, upper=True)
if cluster_type == "G":
...
# ------------------- Photometry config -------------------
DETECTION_SIGMA = 1  # detection threshold in units of background sigma (controls how many stars are detected)
...
# ------------------- Run -------------------
if __name__ == "__main__" and cluster_type in ["O", "G"]:
...
# ---------------------------------------------------------------------
# ------------------- Galaxy light patches mode -------------------
if __name__ == "__main__" and cluster_type.upper().startswith("GAL"):
    try:
        from photutils.segmentation import detect_threshold, detect_sources, source_properties

        def detect_light_patches(data, nsigma=3.0, npixels=20):
            threshold = detect_threshold(data, nsigma=nsigma)
            segm = detect_sources(data, threshold, npixels=npixels)
            if segm is None:
                return [], None
            props = source_properties(data, segm)
            positions = [(p.xcentroid.value, p.ycentroid.value) for p in props]
            return positions, props

        # קריאת התמונות
        data_G = fits.getdata(fits_file_G, ext=0).astype(float)
        data_B = fits.getdata(fits_file_B, ext=0).astype(float)

        # גילוי כתמים
        positions_G, props_G = detect_light_patches(data_G, nsigma=DETECTION_SIGMA, npixels=20)
        positions_B, props_B = detect_light_patches(data_B, nsigma=DETECTION_SIGMA, npixels=20)

        if not positions_G and not positions_B:
            raise RuntimeError("No light patches detected in either band")

        # הפקת טבלה פשוטה: מרכזים + שטח
        rows = []
        for p in props_G:
            rows.append({
                "X": p.xcentroid.value,
                "Y": p.ycentroid.value,
                "Area_pix": p.area.value,
                "Flux_G": p.source_sum.value
            })
        ...
        for p in props_B:
            rows.append({
                ...
                "Y": p.ycentroid.value,
                "Area_pix": p.area.value,
                "Flux_B": p.source_sum.value
            })
        dfB = pd.DataFrame(rows)

        # מיזוג B ו־G על פי קואורדינטות מקורבות
        dfG["X_int"] = dfG["X"].round().astype(int)
        dfG["Y_int"] = dfG["Y"].round().astype(int)
        dfB["X_int"] = dfB["X"].round().astype(int)
        dfB["Y_int"] = dfB["Y"].round().astype(int)

        df = pd.merge(dfG, dfB, on=["X_int", "Y_int"], how="outer")

        out_csv = os.path.join(_outdir, "galaxy_lightpatches.csv")
        df.to_csv(out_csv, index=False)
        print(f"[galaxy] wrote {out_csv} with {len(df)} light patches")
