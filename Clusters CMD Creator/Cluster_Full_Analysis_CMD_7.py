...
def _norm_path(p: str) -> str:
    ...
    if p.lower().startswith("file:///"):
        u = urlparse(p); p = unquote(u.path)
        if len(p)>=3 and p[0]=="/" and p[2]==":": p = p[1:]
    return os.path.normpath(p)

# Import parameter presets
# ================== CONFIG (Interactive, self-contained) ==================
# English only. Press Enter or type 0 to use defaults. This block defines the
# exact variable names the rest of the code expects: fits_file_G, fits_file_B,
# gaia_file, out_csv, out_members, out_png, and all parameter constants.
...
cluster_distance    = _ask("Cluster distance (pc)", 1000.0, float)
cluster_type        = _ask("Cluster type [O=open, G=globular]", "O", str, upper=True)  # O/G

# Photometric extinction/reddening
galactic_extinction = _ask("Galactic extinction A_V", 0.0, float)   # A_V
galactic_reddening  = _ask("Galactic reddening E(B-V)", 0.0, float) # E(B-V)

# Default paths (press Enter to accept). Replace with your typical defaults if needed.
...
fits_file_G = _ask("Path to G- or V-band FITS image", _default_GV, str)  # name kept as fits_file_G for compatibility
...
# Output files (same names as downstream code expects). Saved next to the G/V FITS by default.
...
# ---------------- Detection / photometry parameters (kept names) ----------------
DAO_FWHM           = 5.0
FWHM_BOX_SIZE      = 24
K_APERTURE         = 1.2
K_ANNULUS_IN       = 2.5
...
# distance can be NaN if user entered 0/blank earlier; we keep numeric.
CLUSTER_DISTANCE_PC = float(cluster_distance)
AG_EXTINCTION       = float(galactic_extinction)
EBG_REDDENING       = float(galactic_reddening)


# ---- Preset selection by type (O/G) and distance ----
# Open: distance ≤ 1000 pc -> sparse/nearby; >1000 pc -> moderately distant.
# Globular (G): globular preset.
if cluster_type == "O" and CLUSTER_DISTANCE_PC <= 1000.0:
    # Sparse / nearby open cluster
    # Sparse / nearby open cluster — revised
    DAO_THRESH_SIGMA   = 3.2
    PEAK_SNR_MIN       = 6.0
    MATCH_MAX_ARCSEC   = 3.0
    
    PARALLAX_MIN_HALF  = 0.20
...
elif cluster_type == "O" and CLUSTER_DISTANCE_PC > 1000.0:
    ...
    # Globular cluster (default if not 'O')
    ...
    PARALLAX_FRAC      = 100.0   # avoid choking around tiny parallaxes
    PARALLAX_ERR_SIGMA = 6.0
