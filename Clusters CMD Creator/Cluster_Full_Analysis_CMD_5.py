...
fits_file_G = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\M 13-20230127_G_43.fts"
fits_file_B = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\M 13-20230127_B_43.fts"
gaia_file   = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\1757103795844O-result.csv"

#fits_file_G = r"D:\1 AAA TEMP\Clusters RGB fits\New\NGC 1647\ngc 1647-20250129_G_21.fts"
#fits_file_B = r"D:\1 AAA TEMP\Clusters RGB fits\New\NGC 1647\ngc 1647-20250129_B_21.fts"
#gaia_file   = r"D:\1 AAA TEMP\Clusters RGB fits\New\NGC 1647\1757006309831O-result.csv"

out_csv     = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\photometry_with_membership.csv"
out_members = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\matched_with_membership.csv"
out_png     = r"D:\1 AAA TEMP\Clusters RGB fits\New\M 13\cluster_members_overlay.png"


# ============================================

# ---------------- Detection / photometry parameters (UPDATED) ----------------
DAO_FWHM           = 5.0
DAO_THRESH_SIGMA   = 4.0
PEAK_SNR_MIN       = 10.0
FWHM_BOX_SIZE      = 24

# Use small aperture + aperture correction
K_APERTURE         = 1.2
K_ANNULUS_IN       = 2.5
K_ANNULUS_OUT      = 4.0

# Noise model for uncertainties
GAIN_E_PER_ADU     = 1.0
READ_NOISE_E       = 5.0

# ---------------- CMD / Distance and extinction ----------------
CLUSTER_DISTANCE_PC = 6800
AG_EXTINCTION       = 0.0
EBG_REDDENING       = 0.0

# ---------------- Matching + membership parameters ----------------
#MATCH_MAX_ARCSEC   = 25
#PARALLAX_MIN_HALF  = 0.3
#PARALLAX_ERR_SIGMA = 4.0
#PARALLAX_FRAC      = 0.40
#PARALLAX_MAX_HALF  = 1.0
#PM_SIGMA_FLOOR     = 0.6
#PM_ERR_SIGMA       = 4.0
#PM_CHI2            = 20.0
...
# Catalog-to-image matching (tighten to avoid false matches in crowded fields)
MATCH_MAX_ARCSEC   = 2.0      # was 25

# Parallax window (broaden and relax error dependence)
PARALLAX_MIN_HALF  = 0.05     # was 0.20 mas
PARALLAX_MAX_HALF  = 5.00     # was 1.50 mas
PARALLAX_FRAC      = 100.0    # was 1.00  (effectively removes relative cap)
PARALLAX_ERR_SIGMA = 6.0      # was 5.0

# Proper motions (broader tolerance)
PM_SIGMA_FLOOR     = 1.5      # was 1.0 mas/yr
PM_ERR_SIGMA       = 8.0      # was 5.0
PM_CHI2            = 50.0     # was 30.0

# Cluster "core" definition for reference statistics
SPATIAL_CORE_FRAC  = 0.25     # was 0.20
...
if __name__ == "__main__":
    ...
    hduG  = hdulG[0] if hdulG[0].data is not None else (hdulG[1] if len(hdulG)>1 else hdulG[0])
    dataG = hduG.data
    wcsG = WCS(hduG.header)
    exptimeG = _get_exptime_from_header(hduG.header)
    hdulG.close()
    if dataG is None:
        raise RuntimeError("No image data in G FITS.")

    # ---------- load FITS B ----------
    hdulB = fits.open(fits_file_B)
    hduB  = hdulB[0] if hdulB[0].data is not None else (hdulB[1] if len(hdulB)>1 else hduB[0])
    dataB = hduB.data
    wcsB = WCS(hduB.header)
    exptimeB = _get_exptime_from_header(hduB.header)
    hdulB.close()
    if dataB is None:
        raise RuntimeError("No image data in B FITS.")
    ...
    ra_corr, dec_corr, dx, dy, n_pairs = auto_align_wcs_offsets(
        ...
        gaia['ra'].to_numpy(), gaia['dec'].to_numpy(), search_arcsec=60.0, min_pairs=20
    )
